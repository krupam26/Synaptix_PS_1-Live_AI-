...
# -----------------------------
# Simple RAG Chain (using .invoke to avoid deprecation)
# -----------------------------
...
def rag_invoke(input_dict):
    formatted = rag_prompt.format(**input_dict)
...
def generate(state):
    ...
    state["answer"] = rag_invoke({"question": state["question"], "context": context})
    return state
...
# -----------------------------
# Use BaseRAGQuestionAnswerer with dummy LLM to avoid internal crash
# -----------------------------
print("\n*** EVERYTHING LOADED SUCCESSFULLY ***")
print("*** STARTING SERVER ON PORT 8000 ***\n")

# Dummy RAG to satisfy BaseRAGQuestionAnswerer
dummy_rag = BaseRAGQuestionAnswerer(llm=llm, indexer=document_store)

# Override the default endpoint with our custom agent
@dummy_rag.serve_callable(route="/v1/agent")
...
dummy_rag.build_server(host=HOST, port=PORT)
dummy_rag.run_server()